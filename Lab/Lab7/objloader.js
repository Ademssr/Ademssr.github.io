(function (scope, undefined) {
   ...
   * and collect the vertex, vertex normal, texture, a d face information. This
  ...
  OBJ.Mesh = function (objectData) {
        ...
        */
        ...
        for (var j = 0, eleLen = elements.length; j < eleLen; j++){
            ...
            if(elements[j] in unpacked.hashindices){
                ...
                // vertex position
                /* ========== add by Lyii */
                var x = parseFloat(verts[(vertex[0]-1)*3+0]);
                if( x > unpacked.xmax )
                  unpacked.xmax = x;
                if( x < unpacked.xmin )
                  unpacked.xmin = x;
                var y = parseFloat(verts[(vertex[0]-1)*3+1]);
                if( y > unpacked.ymax )
                  unpacked.ymax = y;
                if( y < unpacked.ymin )
                  unpacked.ymin = y;
                var z = parseFloat(verts[(vertex[0]-1)*3+2]);
                if( z > unpacked.zmax )
                  unpacked.zmax = z;
                if( z < unpacked.zmin )
                  unpacked.zmin = z;
                ...
                // vertex normals
                if( vertNormals.length){
                  unpacked.norms.push(+vertNormals[(vertex[2] - 1) * 3 + 0]);
                  unpacked.norms.push(+vertNormals[(vertex[2] - 1) * 3 + 1]);
                  unpacked.norms.push(+vertNormals[(vertex[2] - 1) * 3 + 2]);
                }else{
                  var localv1 = [+verts[(elements[0]-1)*3+0], +verts[(elements[0]-1)*3+1], +verts[(elements[0]-1)*3+2]];
                  var localv2 = [+verts[(elements[1]-1)*3+0], +verts[(elements[1]-1)*3+1], +verts[(elements[1]-1)*3+2]];
                  var localv3 = [+verts[(elements[2]-1)*3+0], +verts[(elements[2]-1)*3+1], +verts[(elements[2]-1)*3+2]];

                  var lv12 = [localv2[0]-localv1[0], localv2[1]-localv1[1], localv2[2]-localv1[2]];
                  var lv13 = [localv3[0]-localv1[0], localv3[1]-localv1[1], localv3[2]-localv1[2]];
                  ...
                  var len = 1.0 / Math.sqrt(crs[0]*crs[0]+crs[1]*crs[1]+crs[2]*crs[2]);
                  
                  unpacked.norms.push(crs[0]*len);
                  unpacked.norms.push(crs[1]*len);
                  unpacked.norms.push(crs[2]*len);
                }
    ...
    this.xmax = unpacked.xmax;
    this.xmin = unpacked.xmin;
    this.ymax = unpacked.ymax;
    this.ymin = unpacked.ymin;
    this.zmax = unpacked.zmax;
    this.zmin = unpacked.zmin;
  ...
  OBJ.buildBufferExt = function( gl, type, data, itemSize, buffer ){
    var arrayView = type === gl.ARRAY_BUFFER ? Float32Array : Uint16Array;
    gl.bindBuffer(type, buffer);
    gl.bufferData(type, new arrayView(data), gl.STATIC_DRAW);
    buffer.itemSize = itemSize;
